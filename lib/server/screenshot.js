...
export async function screenshotPage(url) {
  if (!SCREENSHOT_API_KEY) throw new Error("ScreenshotAPI key missing");
  ...
  const payload = {
    url,
    output: "base64",         // get base64-encoded image
    device: "desktop",
    full_page: true,
  ...
  const res = await fetch("https://shot.screenshotapi.net/screenshot", {
    method: "POST",
    headers: {
